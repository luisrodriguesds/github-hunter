...
import { FiChevronRight, FiEye, FiStar, FiSettings, FiCircle } from 'react-icons/fi'
...
const Dashboard: React.FC = () => {
  ...
  const [repositories, setRepositories] = useState<IData | null>(null)
  ...
  <>
    ...
    <Form hasError={!!inputError} action="" onSubmit={handleAddRepository}>
      <input type="text" value={newUser} onChange={(e) => setNewUser(e.target.value)} placeholder="Digite o username do Usuário" />
      <button disabled={loadingInput}  type="submit">{loadingInput ? `Carregando...` : `Pesquisar`}</button>
    </Form>
    ...
    <Repositories>
      ...
      {(repositories && !loadingInput) && repositories.data.map(repository => (
        <a key={repository.id} href={repository.html_url} rel="noopener noreferrer" target="_blank">
          <img src={repository.owner.avatar_url} alt="login" />
          <div>
